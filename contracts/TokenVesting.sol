...
contract TokenVesting is Owned, ReentrancyGuard {
    ...
    constructor(
        address token_,
        InitialReciever[] memory _initialRecievers
    ) Owned(msg.sender) {
        // Check that the token address is not 0x0.
        require(token_ != address(0x0));
        // Set the token address.
        _token = ERC20(token_);
        initialTransfer(_initialRecievers);
    }
    ...
    function initialTransfer(
        ...
        for (uint256 i = 0; i < _initialRecievers.length; i++) {
            SafeTransferLib.safeTransfer(
                _token,
                _initialRecievers[i].receiverAddress,
                _initialRecievers[i].amount
            );
        }
    }
